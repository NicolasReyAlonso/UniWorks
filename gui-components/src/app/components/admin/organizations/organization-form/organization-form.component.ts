...
@Component({
    ...
    styleUrls: ['./organization-form.component.sass']
})
export class OrganizationFormComponent implements OnInit {
  ...
  private setOrganizationData(): void {
    if (this.organizationId) {
      this.backendService.getOrganizations(this.organizationId).subscribe((response) => {
  ...
  private formatDataForBackend(requestMethod: 'POST' | 'PUT') {
    ...
    for (const item of identitiesValue) {
      identities.push({organization_id: this.organizationId, identity_id: item});
    }
  ...
  modifyGroup() {
    if (this.formGroup.valid) {
      ...
      this.backendService.putOrganization(this.organizationId, data).subscribe((response) => {
  ...
  createOrganization() {
    if (this.formGroup.valid) {
      ...
      this.disableEdit = true;
      this.backendService.postOrganization(data).subscribe((response) => {
        this.disableEdit = false;
        console.log(response);
