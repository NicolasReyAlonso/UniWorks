...
import {forester} from './forester.js';
//Added by Daniel Reyes
...
(function archaeopteryx() {
    ...
    //TODO: ATTRIBUTE ADDED BY DANIEL REYES
    let _nodesLoadedFirstTime = false;
    ...
    function update(source, transitionDuration, doNotRecalculateWidth) {
        ...
        const extlabel = nodeEnter.append('text')
          .attr('class', 'extlabel')
        ...
        extlabel.filter(function(d) { //Added by Daniel Reyes
          return (d.children || d._children) && typeof(d.name) !== 'undefined' && d.name.includes('/');
        })
        .append('tspan')
        .attr('class', 'extlabel first')
        .text(function (d) {return d.name.split('/')[0];});
        extlabel.filter(function(d) {
          return (d.children || d._children) && typeof(d.name) !== 'undefined' && d.name.includes('/');
        })
        .append('tspan')
        .attr('class', 'extlabel second')
        .text(' / ');
        extlabel.filter(function(d) {
          return (d.children || d._children) && typeof(d.name) !== 'undefined' && d.name.includes('/');
        })
        .append('tspan')
        .attr('class', 'extlabel third')
        ...
        node.select("text.extlabel")
            ...
            .style('fill', '#000000') //Modified by Daniel Reyes
            .attr('dy', function (d) {
        ...
        node.select('circle.nodeCircle')
            ...
            .style('stroke', function (d) {
                //TODO: CODIGO MODIFICADO POR DANIEL REYES
                if(!_nodesLoadedFirstTime)
        ...
        nodeUpdate.select('text.extlabel')//Modified by Daniel Reyes
          .filter(function (d) {
            return (!_options.dynahide || !d.hide) && typeof(d.name) !== 'undefined' && !d.name.includes('/');
          })
            .text(function (d) {
        ...
        link.transition()
            .duration(transitionDuration)
            //TODO: COMMENTED BY DANIEL REYES.attr('stroke', makeBranchColor)
            .attr('d', elbow);
        ...
        window.dispatchEvent(new Event('update_archaeopteryx'));
    }
    ...
    archaeopteryx.launch = function (id,
        ...
        //Linear gradient added by Daniel Reyes
        // Create the svg:defs element and the main gradient definition.
        const svgDefs = _baseSvg.append('defs');
    ...
    function createGui() {
        ...
        if (c0) {
            ...
            //Commented by Daniel Reyes c0.append(makeAutoCollapse());
        ...
        function makeControlButtons() {
            ...
            h = h.concat('<div>');
            // Commented by Daniel Reyes h = h.concat(makeButton('O', ORDER_BUTTON, 'order all (Alt+O)'));
            // Commented by Daniel Reyes h = h.concat(makeButton('R1', RETURN_TO_SUPERTREE_BUTTON_BY_ONE, 'return to supertree by one branch (if in subtree) (Alt+R)'));
            ...
            // Commented by Daniel Reyes h = h.concat(makeButton('M', MIDPOINT_ROOT_BUTTON, 'midpoint re-root (Alt+M)'));
            h = h.concat('</div>');
            h = h.concat('</fieldset>');
            return h;
        }
        ...
        function makeSliders() {
            let h = "";
            h = h.concat('<fieldset>');
            h = h.concat('<legend>Sizes</legend>');//Added by Daniel Reyes
            h = h.concat('<div style="font-size:10px;">');//Edited by Daniel Reyes
    ...
    function makeBackground() {
        _baseSvg.append('rect')
            ...
            .attr('fill', '#ffffff'); // Changed by Daniel Reyes
    }
