...
export class CanEnterPage  {
    ...
    canActivateChild(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<boolean> {
        console.log('🔐 CanEnterPage: Verificando permisos...');
        
        // Esperar a que authInit sea true antes de verificar permisos
        return this.authService.authInit$.pipe(
            ...
            map(() => {
                ...
                // Permission is checked by the backend when loading
                // the navigation tree, so if the route was served by
                // the backend it is already authorised. We still do
                // a best-effort client-side check if screenDefinition
                // carries a permission field.
                if (path === 'd') {
                    const screenName = route.params?.['screenName'] ?? route.url?.[1]?.path;
                    if (screenName) {
                        // The resolver will load the screen; the backend
                        // already filtered menus by the user's permissions
                        // so reaching this point means the user has access.
                ...
                switch (path) {
                    ...
                    case 'processesBrowseTodDo':
                        if (!(route.queryParams && route.queryParams["status"] && 
                            (route.queryParams["status"] == "todo" || route.queryParams["status"] == "done"))) {
                            canEnter = false;
                            break;
                        }
